...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    masked_scores = masked_scores / self.attention_head_size**0.5
    normalized_scores = torch.softmax(masked_scores, dim = -1)
    
    output = torch.matmul(normalized_scores, value)
    output = output.transpose(1, 2)
    bs, seq_len, heads, head_size = output.shape
    output = output.reshape(bs, seq_len, heads * head_size)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    # Get the attention layer output 
    attention = self.self_attention(hidden_states, attention_mask)

    interm = self.add_norm(hidden_states, attention, dense_layer= self.attention_dense, dropout=self.attention_dropout, ln_layer=self.attention_layer_norm)
    
    x = self.interm_af(self.interm_dense(interm))
    
    output = self.add_norm(interm, x, dense_layer=self.out_dense, dropout= self.out_dropout, ln_layer= self.out_layer_norm)
